...
export default function Home() {
  const apiUrl = "https://quicknoteapi.emrahsozlu.com/api/Notes";
  ...
  const leftColumn = (
    <>
      ...
      <ListGroup className='mt-2'>
        {
          notes.map(note => (
            <ListGroup.Item key={note.id} action active={note.id == selectedNote?.id} onClick={(e) => itemClicked(e, note)}>
              {note.title}
            </ListGroup.Item>
          ))
        }
    ...
    <main className={styles.main}>
      ...
      <Offcanvas show={show} onHide={handleClose}>
        <Offcanvas.Header closeButton>
          <Offcanvas.Title>QuickNote</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
          {leftColumn}
        </Offcanvas.Body>
